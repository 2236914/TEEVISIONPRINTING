...
type SecondSectionTypes = {
  homepageLongSleeveShirtProducts: Array<HomePageProducts>;
  homepageSweatshirtProducts: Array<HomePageProducts>;
  homepageTshirtProducts: Array<HomePageProducts>;
};

const PRODUCT_LIMIT = 16;

const SecondSection: React.FC<SecondSectionTypes> = ({
  ...
  const productsBasedOnTab = useMemo(() => {
    let selectedProducts: HomePageProducts[] = [];
    if (tabSelected === 0) {
          ...
          <div className="flex flex-col gap-4 xl:flex-row justify-between items-start p-8 md:pt-16 xl:pt-24 max-w-[90rem] w-full">
            ...
            <div className="flex flex-col xl:w-[30rem] gap-8 xl:items-end">
              <p className="text-white xl:text-right text-md md:text-2xl xl:text-[1rem] 1_5xl:text-lg">
                A glimpse into our most popular custom apparel, loved for their
